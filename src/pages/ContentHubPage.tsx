...
export const ContentHubPage: React.FC = () => {
  ...
  const active = useMemo<MetaChannel>(() => {
    ...
    if (fromUrl === 'instagram' || fromUrl === 'facebook') return fromUrl;
    const saved = localStorage.getItem(STORAGE_KEY);
    return saved === 'instagram' ? 'instagram' : 'facebook';
  }, [searchParams]);
  ...
  const changeChannel = (channel: MetaChannel) => {
    ...
    if (channel === 'instagram') next.delete('status');
  ...
  return (
    <div className="space-y-3 min-w-0">
      <div className="flex items-center justify-between gap-3">
        ...
        <MetaChannelTabs active={active} onChange={changeChannel} facebookLabel="Facebook" instagramLabel="Instagram" />
      </div>

      {active === 'facebook' ? <ContentListPage /> : <InstagramContentWorkspace />}
    </div>
  );
};
